...
{
	private const string TestMessage = "Test out of range argument error message";
	...
	private const string TestActualValue = "TestActualValue";
	private const string CustomKey = "CustomKey";
	private const string CustomValue = "CustomValue";
	private const string ExpectedCode = "ARG_OUT_OF_RANGE";
	...
	{
		// Arrange & Act
		var exception = new ArgOutOfRangeException(TestMessage);
	...
	public void Constructor_WithMessageAndInnerException()
	{
		...
		// Act
		var exception = new ArgOutOfRangeException(TestMessage, innerException);
		...
		Assert.That(exception.Line, Is.Null);
		Assert.That(exception.Param, Is.Null);
		Assert.That(exception.ActualValue, Is.Null);
	}
	...
	public void Constructor_WithInnerExceptionOnly()
	{
		...
		// Act
		var exception = new ArgOutOfRangeException(innerException);
	...
	public void Constructor_WithAllPropertiesSet()
	{
		...
		Assert.That(exception.ActualValue, Is.EqualTo(TestActualValue));
	...
	public void ToString_WithAllPropertiesSets_IncludesAllDetail()
	{
		...
		Assert.That(toStringResult, Does.Contain(TestActualValue));
	...
	public void ToString_WithEmptyProperties_ExcludesEmptyValues()
	{
		...
		Assert.That(toStringResult, Does.Not.Contain("ActualValue:"));
	}
	...
	public void ToString_WithComplexExceptionChain_IncludesAllExceptions()
	{
		...
		var level1Exception = new ArgOutOfRangeException(TestMessage, level2Exception);
	...
	{
		// Arrange
		ArgOutOfRangeException exception;
		try
		{
			throw new ArgOutOfRangeException(TestMessage);
		}
		catch (ArgOutOfRangeException ex)
		{
			exception = ex;
		}
		...
		// Assert
		Assert.That(toStringResult, Does.Contain("--ArgOutOfRangeException"));
	...
	public void ToString_WithPredefinedKeys_IncludesOnlyNonEmptyValues()
	{
		...
		Assert.That(toStringResult, Does.Contain($"ActualValue: \"{TestActualValue}\""));
	}
	...
	public void Inheritance_FromDetailedException_WorksCorrectly()
	{
		...
		Assert.That(exception, Is.InstanceOf<ArgOutOfRangeException>());
	}

	[Test]
	public void Code_IsAlwaysSetToArgOutOfRange()
	{
		// Arrange & Act
		var exception1 = new ArgOutOfRangeException(TestMessage);
		var exception2 = new ArgOutOfRangeException();
		var exception3 = new ArgOutOfRangeException(new Exception());
